...
-- So let's do outliers detection
...
SELECT Version, SUM(Gross_Profit) AS US_totalprofit
FROM CarSales
WHERE Country = 'US'
GROUP BY Version;																		
...
SELECT Version, COUNT(*) AS no_sal_trans
FROM CarSales
WHERE Country = 'US'
GROUP BY Version;																	    
...
SELECT Purchase_type, ROUND(AVG(Price), 3) AS avg_sales
FROM CarSales
WHERE Country = 'US'
GROUP BY Purchase_type;																 
