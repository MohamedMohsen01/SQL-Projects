...
select type, 
   ROUND(AVG(size), 3) as avg_size, 
   ROUND(AVG(weekly_sales), 3) as total_sales 
from 
  sales as a 
  join stores as b 
	on a.store = b.store 
...
WITH new AS (
  SELECT 
    ...
    b.Weekly_Sales 
  FROM sales as b 
    JOIN stores as a ON a.Store = b.Store
) 
SELECT 
  Type, 
  ROUND(AVG(size),3) as avg_size, 
  ROUND(AVG(Weekly_Sales),3) as avg_weekly_sales 
FROM 
  new 
GROUP BY 
  Type;
...
SELECT 
  Store, 
  ROUND(SUM(Weekly_Sales), 3) as annual_sales 
FROM 
  sales 
WHERE 
  Date BETWEEN '2010-02-05' AND '2011-02-05' 
GROUP BY 
  Store 
ORDER BY 
  annual_sales DESC;
...
GROUP BY 
	Store
ORDER BY 
	total_sales DESC;  --Our best performing Store is 20 with total_sales = 301397792.45999974
...
GROUP BY 
	Store
ORDER BY 
	total_sales ASC;  --Our worst performing Store is 33 with total_sales = 37160221.96
...
SELECT 
	Store, 
	ROUND(AVG(Weekly_Sales), 3) as average_sales, IsHoliday
FROM 
	sales
GROUP BY 
	Store, 
...
SELECT 
    AVG(total_sal) as average_sales, 
    unemployment_cat 
  FROM 
    unemployment_CTE 
...
SELECT 
    AVG(total_sal) as [average sales], 
    fuel_cat 
  FROM 
    fuel 
  GROUP BY 
...
WITH CPI AS (
  SELECT 
    date, 
    Total_sal, 
    CASE WHEN CPI > 129 THEN 'high' WHEN CPI <= 129 
    AND CPI > 127.5 THEN 'middle' ELSE 'low' END AS CPI_cat 
  FROM 
    #combined_33
) 
SELECT 
  AVG(total_sal) as [average sales], 
  CPI_cat, 
  COUNT(CPI_cat) as num_of_records 
FROM 
  CPI 
GROUP BY 
  CPI_cat;
...
FROM 
  features_20 AS a 
  JOIN store_20 AS b ON a.date = b.Date;
...
WITH unemployment AS (
  SELECT 
    date, 
    total_sal, 
    CASE WHEN Unemployment < 7.8 THEN 'low' WHEN Unemployment >= 8 
    AND Unemployment < 9.5 THEN 'middle' ELSE 'high' END AS unemployment_cat 
  FROM 
    #combined_20)
  SELECT 
    AVG(Total_sal), 
...
WITH fuel AS (
  SELECT 
    date, 
    total_sal, 
    CASE WHEN Fuel_Price > 4.1 THEN 'high' WHEN Fuel_Price <= 4.1 
    AND Fuel_Price > 3.1 THEN 'middle' ELSE 'low' END AS fuel_cat 
  FROM 
    #combined_20)
  SELECT 
    AVG(total_sal) as [average sales], 
...
With fuel AS (
  SELECT 
    date, 
    total_sal, 
    CASE WHEN Fuel_Price > 3.7 THEN 'high' WHEN Fuel_Price <= 3.7 
    AND Fuel_Price > 3.0 THEN 'middle' ELSE 'low' END AS fuel_cat 
  FROM 
    #combined_20)
  SELECT 
    AVG(total_sal) as [average sales], 
...
WITH CPI AS (
  SELECT 
    date, 
    total_sal, 
    CASE WHEN CPI > 212 THEN 'high' WHEN CPI <= 212 
    AND CPI > 207 THEN 'middle' ELSE 'low' END AS CPI_cat 
  FROM 
    #combined_20)
  SELECT 
    AVG(total_sal) as [average sales], 
